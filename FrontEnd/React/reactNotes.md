...
Data such as this, which a component itself owns, is called `state`. State is another powerful tool for React because components not only **own** state, but can **update** it later. _It's not possible to update the [props](#prop) a component receives; only to read them._
...
In original scenarios, when user click on a button to jump into a new page, the request is send to the _Server side_, and request a new page to the client to render. Clients are allowed to jump from pages to pages, and all pages have different paths. This is called `Server-Side-Routing`.
